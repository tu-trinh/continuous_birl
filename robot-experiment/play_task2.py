...
def main():
    ...
    optimals = pickle.load( open( "choices_task2/optimal.pkl", "rb" ) )
    ...
    while True:
        ...
        if curr_t > total_time * 0.420 and not picked_up:
            send2gripper(conn_gripper)
            pause_t = time.time()
            start_t += 2.0
            while time.time() - pause_t < 2.0:
                picked_up = True

        if play_traj:
            curr_t = time.time() - start_t
            q_des = traj.get(curr_t)
            qdot = 10 * (q_des - state["q"])
            send2robot(conn, qdot)
            if curr_t - total_time > 2.0 and not dropped:
                send2gripper(conn_gripper)
                dropped = True
            if curr_t - total_time > 3.0:
                return True
