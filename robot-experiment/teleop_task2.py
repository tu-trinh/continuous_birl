 ...
 * includes noise to make things harder
...
def main():
    ...
    interface = Joystick()
    foldername = "demos_task2/user" + usernumber + "/"
    ...
    last_time = None
    stack_success = 0
    prev_rand_time = time.time()
    rand_action = np.asarray([0.0]*6)
    gripper_closed_time = None
    ...
    while True:
        ...
        state = readState(conn)
        z, grasp, success, stop = interface.input()
        ...
        if gripper_closed and record and (time.time() - gripper_closed_time) > 1.0:
            if (time.time() - prev_rand_time) > hold_time:
                prev_rand_time = time.time()
        ...
        if record and curr_time - last_time >= steptime:
            demonstration.append(state["q"].tolist() + qdot.tolist() + [stack_success])
            last_time = curr_time
