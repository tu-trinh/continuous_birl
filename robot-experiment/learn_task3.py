...
def birl_belief(beta, D, O):
    ...
    opt_c1 = trajcost(O["0.0"][0], THETA[0])
    opt_c2 = trajcost(O["0.5"][0], THETA[1])
...
def main():
    ...
    for usernumber in range(1,11):
        ...
        folder = "demos_task3/user"+usernumber
        demos = []
        ...
        for episode in range(N_SPARSE):
            ...
            duration = target + np.random.randint(100,200)
            xi1 = hold(xi, target, duration)
            S.append(rescale(xi1, N_WAYPOINTS, 1))

        savefolder = "choices_task3/user"+usernumber
        ...
        O = pickle.load( open( "choices_task3/optimal.pkl", "rb" ) )
    ...
    pickle.dump(belief, open("results/belief_task3.pkl", "wb") )
